...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check for duplicate destination in the same country
    if (empty($errors)) {
        try {
            $stmtCheck = $pdo->prepare('SELECT COUNT(*) FROM destinations WHERE LOWER(destination_name) = LOWER(:name) AND LOWER(country) = LOWER(:country)');
            $stmtCheck->execute([
                ':name'    => $name,
                ':country' => $country,
            ]);
            if ((int)$stmtCheck->fetchColumn() > 0) {
                $errors[] = "A destination named \"{$name}\" in \"{$country}\" already exists.";
            }
    ...
    // Insert record if validation passes
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare('
                INSERT INTO destinations (destination_name, country, description, status, created_at)
                VALUES (:name, :country, :description, :status, NOW())
            ');
            $stmt->execute([
                ':name'        => $name,
                ':country'     => $country,
            ...
            set_flash_message('success', "Destination \"{$name}\" added successfully!");
            header('Location: ' . url('admin/destinations/index.php'));
            exit;
        } catch (PDOException $e) {
            $errors[] = 'Failed to create destination: ' . $e->getMessage();
        }
    }
}
...
<div class="container-fluid p-0">
    ...
    <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between mb-4 pb-2 border-bottom">
        <div>
            ...
            <h3 class="fw-bold mb-0 text-dark">Add New Destination</h3>
        </div>
        ...
        <div class="col-12 col-lg-8">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-white py-3">
                    <h5 class="fw-bold mb-0 text-dark"><i class="bi bi-pin-map text-primary me-2"></i>Destination Details</h5>
                </div>
                <div class="card-body p-4">
                    <form action="<?= url('admin/destinations/create.php') ?>" method="POST" autocomplete="off">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <input type="text"
                                       ...
                                       placeholder="e.g. Bali Island, Swiss Alps, Kyoto"
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <input type="text"
                                       ...
                                       id="country"
                                       name="country"
                                       placeholder="e.g. Indonesia, France, Japan, India"
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control"
                                          ...
                                          name="description"
                                          rows="4"
                                          placeholder="Provide a captivating overview of highlights, sights, and climate..."><?= htmlspecialchars($description, ENT_QUOTES, 'UTF-8') ?></textarea>
